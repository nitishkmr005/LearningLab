...
def analyze_ab_test(
    ...
    ci      = (abs_lift - z_crit * se, abs_lift + z_crit * se)
    ...
    return {
        ...
        "ci_95_pp":          (round(ci[0] * 100, 2), round(ci[1] * 100, 2)),
    }
...
# → srm_detected: False  (assignment is clean)

result = analyze_ab_test(
    n_control=10_000,   clicks_control=2_100,
    n_treatment=10_000, clicks_treatment=2_520,
)
...
SRM check: {'observed_split': 0.5, 'chi2': 0.0, 'p_value': 1.0, 'srm_detected': False}
  ...
  control_ctr:       0.21
  treatment_ctr:     0.252
  absolute_lift_pp:  4.2        ← +4.2 percentage points
  relative_lift_pct: 20.0       ← +20% relative lift
  z_stat:            7.047
  p_value:           0.000002   ← p < 0.0001, highly significant
  significant:       True
  ci_95_pp:          (3.03, 5.37)   ← 95% CI: [+3.0 pp, +5.4 pp]
```

**Reading the output:** The treatment CTR (25.2%) beats control (21.0%) by +4.2 percentage points (+20% relative). The z-test confirms this is not random noise (p ≈ 0). The 95% CI [+3.0, +5.4 pp] tells you the range of true effects — the entire interval is positive, so you can ship with confidence. SRM = 0 confirms the randomization was clean.
