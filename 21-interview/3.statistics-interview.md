...
- **Cross-Validation:** In K-fold CV, each fold produces one performance score. The mean of those K scores is a sample mean. CLT justifies treating it as normally distributed, enabling a confidence interval: mean ± 1.96 × std(scores) / √K. Without CLT you couldn't put error bars on CV results.
...
## Q25: What statistical tests are used for feature selection, and when do you use each?

Feature selection statistics answer: "Does this feature carry information about the target, beyond what I'd expect by chance?" The right test depends on the feature type and the target type.

### Decision guide

| Feature type | Target type | Test | sklearn |
|---|---|---|---|
| Categorical | Categorical | Chi-square (χ²) | `chi2` |
| Continuous | Categorical (binary/multi) | ANOVA F-test | `f_classif` |
| Continuous | Continuous | Pearson correlation / F-test | `f_regression` |
| Any | Any | Mutual information | `mutual_info_classif` / `mutual_info_regression` |
| Continuous | Binary | Point-biserial correlation | `scipy.stats.pointbiserialr` |
| Any | Binary | Information Value (IV) / WoE | custom / `scorecardpy` |
| Continuous | Any | VIF (multicollinearity) | `statsmodels.stats.outliers_influence` |

---

### ANOVA F-test (continuous feature → categorical target)

ANOVA tests whether the mean of a continuous feature differs significantly across target classes. A high F-statistic means the feature separates the classes well.

$$F = \frac{\text{Between-group variance}}{\text{Within-group variance}} = \frac{SS_B / (k-1)}{SS_W / (n-k)}$$

where k = number of classes, n = total samples. Under H₀ (feature means are equal across all classes), F follows an F-distribution with (k−1, n−k) degrees of freedom.

**Worked example:** Feature = `account_age`, Target = {churn, no-churn}. If churners average 6 months account age and non-churners average 24 months, F will be large → feature is useful.

**Assumption:** Normality within each group, equal variances across groups (homoscedasticity). Robust to violations when groups are large.

---

### Mutual Information (MI)

MI measures the reduction in uncertainty about Y after knowing X, without assuming any functional form:

$$I(X; Y) = \sum_{x,y} p(x,y) \log \frac{p(x,y)}{p(x)p(y)}$$

- MI = 0 → X and Y are independent (feature is useless)
- MI > 0 → knowing X reduces uncertainty about Y (regardless of whether the relationship is linear, quadratic, or anything else)

**When to use over F-test / chi-square:** when the feature-target relationship is non-linear (e.g., quadratic, step-function). ANOVA F-test and chi-square only detect linear/monotonic associations; MI catches any dependency.

**Limitation:** MI scores are not on an interpretable scale (not bounded) — use them for ranking features, not for absolute interpretation.

---

### VIF (Variance Inflation Factor) — multicollinearity detection

VIF measures how much a feature's variance is inflated by its correlation with other features. For feature $j$, regress $X_j$ on all other features:

$$\text{VIF}_j = \frac{1}{1 - R_j^2}$$

where $R_j^2$ is the R² of that regression.
...
| > 10 | Severe — remove or combine feature |

**When VIF matters:** only for models that estimate coefficients and require them to be interpretable (linear regression, logistic regression, Cox regression). Tree-based models (XGBoost, LightGBM, Random Forest) are unaffected by multicollinearity.
...
# Remove features with VIF > 10 one at a time, recalculate after each removal
```

**Where it's used in practice:**
- **ML:** Chi-square and ANOVA/MI are the standard filter-method feature selection tools in sklearn pipelines before training XGBoost or logistic regression; VIF is mandatory before any linear regression-based model
- **Recommenders:** MI used to rank user behavioral features before training ranking models
- **NLP:** Chi-square for selecting informative n-grams in text classification; MI for selecting features from high-dimensional TF-IDF matrices; in LLM fine-tuning, MI between input tokens and output quality is used to select training examples

**Importance & what goes wrong without it:**
- **Why it matters:** Selecting features without statistical testing includes noise features that increase model variance and training time without improving performance; using the wrong test (chi-square on continuous data) gives invalid rankings
- **What goes wrong:** Including VIF > 10 features in logistic regression makes coefficient signs flip unpredictably (the sign of correlated features can be arbitrarily assigned by the optimizer); relying on ANOVA F-test alone for non-linear features misses predictive features that have no linear relationship with the target (e.g., a U-shaped relationship between age and churn); a feature with near-perfect correlation with the target (|r| > 0.95) is almost always a post-event feature leaking the outcome into training
