...
**Sampling & Bias** — Population vs. Sample · Sampling Techniques · Sampling Bias · Training Data
...
- [Q16: What sampling strategies should you use when building a training dataset for XGBoost?](#q16-what-sampling-strategies-should-you-use-when-building-a-training-dataset-for-xgboost)
- [Q17: How do you handle class imbalance in training data for XGBoost?](#q17-how-do-you-handle-class-imbalance-in-training-data-for-xgboost)
...
## Q16: What sampling strategies should you use when building a training dataset for XGBoost?
...
## Q17: How do you handle class imbalance in training data for XGBoost?
...
| Question | Test | Key Assumption | ML Use Case |
|---|---|---|---|
| Two means different? (large n) | Z-test | Known σ or n > 30 | A/B test with large sample |
| Two means different? (small n / unequal size) | Welch's t-test | Independence | Model A vs B on CV folds; unequal A/B groups |
| Three or more means different? | ANOVA + Tukey HSD post-hoc | Normality, equal variance | Compare 3+ model variants |
| Two proportions different? | Two-proportion z-test | n·p > 5 | A/B test on click or conversion rate |
| Linear feature–target relationship? | Pearson correlation | Both continuous, linear | Feature selection for linear models |
| Monotonic feature–target relationship? | Spearman rank correlation | Both ordinal/continuous | Feature selection for tree models |
| Categorical feature associated with target? | Chi-square test | Expected cell freq > 5 | Categorical feature selection |
| Distribution shifted from baseline? | KS test / PSI | Continuous feature | Drift detection in production |
| Multicollinearity present? | VIF | OLS assumptions | Pre-flight before linear / logistic regression |
| Time series is stationary? | ADF (Augmented Dickey-Fuller) | Time series | Pre-flight before ARIMA / lag features |
| Paired before/after comparison? | Paired t-test / Wilcoxon | Same subjects observed twice | Same-user before–after experiment |
| Non-parametric group comparison? | Mann-Whitney U | Independence | Robustness check when normality fails |
