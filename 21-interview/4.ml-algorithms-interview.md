...
$$TPR = \frac{TP}{TP + FN} \quad \text{(of all actual positives, how many did we catch?)}$$

**False Positive Rate (FPR):**
$$FPR = \frac{FP}{FP + TN} \quad \text{(of all actual negatives, how many did we falsely flag?)}$$

**ROC Curve:** plot of **TPR (y-axis) vs. FPR (x-axis)** as you sweep the classification threshold from 1 → 0. At threshold=1 every sample is called negative → (0,0). At threshold=0 every sample is called positive → (1,1).

---

### How the ROC curve is built — step-by-step construction

**Dataset: 5 samples** (2 cancer patients, 3 healthy). Model outputs a probability score.
...
**Step 1 — Sort by score descending.** Order: A(0.9), B(0.8), C(0.6), D(0.4), E(0.1)

**Step 2 — Sweep threshold: at each threshold, flag everything ≥ threshold as positive.**

| Threshold | Flagged as + | TP | FP | FN | TN | **TPR** | **FPR** |
|---|---|---|---|---|---|---|---|
| > 0.90 | none | 0 | 0 | 2 | 3 | **0.00** | **0.00** |
| ≥ 0.90 | A | 1 | 0 | 1 | 3 | **0.50** | **0.00** |
| ≥ 0.80 | A, B | 1 | 1 | 1 | 2 | **0.50** | **0.33** |
| ≥ 0.60 | A, B, C | 2 | 1 | 0 | 2 | **1.00** | **0.33** |
| ≥ 0.40 | A, B, C, D | 2 | 2 | 0 | 1 | **1.00** | **0.67** |
| ≥ 0.10 | all | 2 | 3 | 0 | 0 | **1.00** | **1.00** |

**Step 3 — Plot each (FPR, TPR) point and connect them.**
...
0.0 ●───┼────┬────────────────┬────────┬── FPR
  ...
  ● points: (0,0) → (0,0.5) → (0.33,0.5) → (0.33,1.0) → (0.67,1.0) → (1.0,1.0)
  
  Random classifier: diagonal from (0,0) to (1,1)
  Perfect classifier: up the left edge (0,0)→(0,1) then right (0,1)→(1,1)
```

**Intuition:** Each step right = one new false positive added. Each step up = one new true positive added. A good model catches all the positives (steps up) before accumulating false positives (steps right) — so the curve hugs the top-left corner.
...
AUC is the area under the ROC curve — a single number summarising discrimination ability across **all possible thresholds**.

**Calculating AUC with the trapezoid rule** (continuing the 5-sample example above):

Each segment between consecutive ROC points contributes: area = width × height (for rectangles, since our curve is staircase-shaped):

| Segment | Width (ΔFPR) | Height (TPR) | Area |
|---|---|---|---|
| (0,0) → (0,0.5) | 0 | — | 0.000 |
| (0,0.5) → (0.33,0.5) | 0.33 | 0.50 | **0.165** |
| (0.33,0.5) → (0.33,1.0) | 0 | — | 0.000 |
| (0.33,1.0) → (0.67,1.0) | 0.34 | 1.00 | **0.340** |
| (0.67,1.0) → (1.0,1.0) | 0.33 | 1.00 | **0.330** |

$$\text{AUC-ROC} = 0.165 + 0.340 + 0.330 = \mathbf{0.835}$$
...
**Probabilistic interpretation:** AUC = probability that the model ranks a randomly chosen positive *higher* than a randomly chosen negative.

In our example AUC = 0.835: if you pick one cancer patient and one healthy patient at random, the model assigns a higher score to the cancer patient 83.5% of the time. You can verify by hand: there are 2 × 3 = 6 (positive, negative) pairs: (A,B), (A,D), (A,E), (C,B), (C,D), (C,E). The model scores A > B ✓, A > D ✓, A > E ✓, C < B ✗, C > D ✓, C > E ✓ → 5/6 = 0.833 ≈ 0.835.

**AUC as a measure of class separation:**

The AUC score is a direct, quantitative measure of how *separable* your positive and negative classes are. When you look at an AUC score, you are looking at how much **overlap** exists between the probability score distributions of your positive class and your negative class.

```
AUC ≈ 1.0 — no overlap, perfect separation:
...
AUC ≈ 0.8 — small overlap, model is fairly good:
...
AUC ≈ 0.5 — complete overlap, model is guessing:

  Negatives and Positives fully mixed
    ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓
   ──┼──────────────┼────▶ score
     0              1
```

**Interviewer favourite:** "If two models have AUC 0.85 and 0.75, what does that mean?" → The 0.85 model's score distributions for positives and negatives have less overlap — it assigns higher scores to actual positives more consistently, so any threshold you pick will give better trade-offs.
...
### The Precision-Recall curve — how it's built

The PR curve uses the **same sorted threshold sweep** as the ROC curve, but plots **Precision (y-axis) vs. Recall (x-axis)** at each threshold.

**Continuing the same 5-sample example** (A+:0.9, B−:0.8, C+:0.6, D−:0.4, E−:0.1):

| Threshold | TP | FP | FN | **Recall** (TPR) | **Precision** |
|---|---|---|---|---|---|
| ≥ 0.90 | 1 | 0 | 1 | 1/2 = **0.50** | 1/1 = **1.00** |
| ≥ 0.80 | 1 | 1 | 1 | 1/2 = **0.50** | 1/2 = **0.50** |
| ≥ 0.60 | 2 | 1 | 0 | 2/2 = **1.00** | 2/3 = **0.67** |
| ≥ 0.40 | 2 | 2 | 0 | 2/2 = **1.00** | 2/4 = **0.50** |
| ≥ 0.10 | 2 | 3 | 0 | 2/2 = **1.00** | 2/5 = **0.40** |

PR curve points (Recall, Precision): (0.5, 1.00) → (0.5, 0.50) → (1.0, 0.67) → (1.0, 0.50) → (1.0, 0.40)
...
0.0 │             │ │ │
  ...
  Perfect classifier: stays at (0,1) then goes to (1,1)
  Random baseline: horizontal line at class_rate = 2/5 = 0.40
```

**Intuition:** As you lower the threshold to catch more positives (Recall goes right), you inevitably pull in more false positives — Precision drops. The PR curve traces this exact trade-off. A good model keeps Precision high even as Recall increases → curve stays near the top-right.

**AUC-PR (Average Precision):** Area under the PR curve, computed as a weighted mean of Precision at each Recall step:

$$AP = \sum_k \text{Precision}(k) \times \Delta\text{Recall}(k)$$

For our example:
- At Recall 0→0.5: Precision = 1.00, ΔRecall = 0.5 → contribution = **0.50**
- At Recall 0.5→1.0: Precision = 0.67, ΔRecall = 0.5 → contribution = **0.335**
- $$\text{AUC-PR} \approx 0.50 + 0.335 = \mathbf{0.835}$$

*(For this balanced toy dataset both AUC-ROC and AUC-PR coincide — the divergence appears clearly at extreme imbalance.)*

---

### Why AUC-ROC can lie on imbalanced data

**AUC-ROC** can be overly optimistic with extreme class imbalance. FPR uses TN in the denominator:

$$FPR = \frac{FP}{FP + TN}$$

With millions of true negatives (e.g., fraud at 0.1%), even 10 000 false positives produce a tiny FPR of 0.0001. The ROC curve looks spectacular while the model is nearly useless in practice.

**AUC-PR** uses no TN term anywhere — every metric (Precision, Recall) is anchored to the positive class. A model that misses most frauds can't hide behind the vast majority of correct "not fraud" calls.

| | AUC-ROC | AUC-PR |
|---|---|---|
| What it measures | How well scores rank positives above negatives | How well the model retrieves positives with high precision |
| Sensitive to class imbalance | **No** — TN in FPR absorbs imbalance | **Yes** — exposes weak positive-class performance |
| Baseline (random) | 0.50 always | = positive class rate (e.g., 0.01 for 1% fraud) |
| Prefer when | Balanced classes, need threshold-agnostic ranking | Rare positive class, cost of FP matters |

**When to use which:**

| Scenario | Use |
|---|---|
| Balanced classes (50/50 or close) | AUC-ROC |
| Rare positives (< 5% — fraud, disease, anomaly) | AUC-PR |
| You will rank outputs (recommendation, search) | AUC-ROC or AUC-PR depending on positives density |
| You care about FP cost (spam filter) | AUC-PR |
| You care about FN cost (cancer screening) | Recall at fixed threshold, or F1 |
| Business wants one number across all thresholds | AUC-PR for imbalanced, AUC-ROC for balanced |

**The honest fraud story:** A fraud model at 0.1% rate might show AUC-ROC = 0.98 (looks great!) but AUC-PR = 0.35 (struggles badly to find rare fraud). Only AUC-PR reveals the real picture.
