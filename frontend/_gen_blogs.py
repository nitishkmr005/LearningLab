...
STYLE = r"""
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .toc-list a.toc-active { color: var(--accent); border-left-color: var(--accent); }
  .toc-list .toc-h3 a { font-size: 11px; padding-left: 24px; color: var(--faint); }
  .toc-list .toc-h3 a.toc-active { color: var(--accent); border-left-color: var(--accent); }
...
window.addEventListener('load', function() {{
  ...
  try {{
    ...
    if (headings.length > 1) {{
      ...
      var links     = Array.from(tocEl.querySelectorAll('a'));
      var activeIdx = -1;

      function updateToc() {{
        var newIdx = -1;
        for (var i = headings.length - 1; i >= 0; i--) {{
          if (headings[i].getBoundingClientRect().top <= 110) {{ newIdx = i; break; }}
        }}
        if (newIdx !== activeIdx) {{
          activeIdx = newIdx;
